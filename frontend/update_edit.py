...
edit_jsx = r'''import { useState, useEffect, useContext } from 'react';
...
function EditCenter() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.patch(/business-centers//, data, {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={sectionClass}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label className={labelClass}>Umumiy maydoni (m2)</label>
          ...
          <div className={sectionClass}>
             ...
             {isAddingNewAdmin ? (
              ...
              <div className="border border-slate-200 rounded-xl p-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-[300px] overflow-y-auto">
                 {users.map(u => (
                   <label key={u.id} className={lex items-center p-3 rounded-xl cursor-pointer transition-all border }>
